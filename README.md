...
6. **1枚目の画像の左上に、樹種名と商品番号(例:「ヤマザクラ SKR-505」)を自動で描画する**
...
10. Instagram Graph API で投稿(画像が2枚以上ある場合は複数枚投稿=カルーセル)
11. 投稿済みの商品IDは `images/posted/posted_auction_ids.txt` に記録され、次回以降は選ばれない
12. 手動追加した画像を使った場合は、`images/posted/` に移動し `posted_log.csv` に記録
