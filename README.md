...
1. 毎日 **20:00 (JST)** に GitHub Actions が起動する
...
5. 画像を Cloudinary にアップロードして公開URLを取得
6. Claude(画像認識)が、取得した正確な情報をもとにキャプション+ハッシュタグを自動生成
7. Instagram Graph API で投稿(画像が2枚以上ある場合は複数枚投稿=カルーセル)
8. 投稿済みの商品IDは `images/posted/posted_auction_ids.txt` に記録され、次回以降は選ばれない
9. 手動追加した画像を使った場合は、`images/posted/` に移動し `posted_log.csv` に記録

出品中の商品をすべて投稿し終えると、その日は「投稿対象なし」としてスキップされます(在庫を増やせば、また自動的に選ばれるようになります)。

### 投稿時間について

20:00 JSTは、平日・休日を問わず夕食後〜就寝前のリラックスタイムにあたり、一般的にInstagramの利用がもっとも伸びやすい時間帯のため初期値にしています。ただし、これはあくまで一般論の目安です。
