...
```
videos/queue/01.mp4
videos/queue/01.txt   ← (任意)ヤフオクURLや樹種メモ。画像と同じ仕組み
```
...
実行のたびに `state/heartbeat.json` に結果を書き残します。投稿が成功したかどうかに
関わらず「いつ動いたか」を残すのが目的です。**これが無いと、仕組みが止まっていても
外から気づけません。**

```json
{
  "at": "2026-09-13T20:05:12",
  "status": "posted",
  "detail": "media_id=18111000553807110 スライドショー(10枚)",
  "host": "JUNSHIN-PC"
}
```
...
- `posted` — 投稿した
- `skipped` — 動いたが投稿対象がなかった(出品中の商品をすべて投稿済み等。異常ではない)
- `error` — 失敗した

履歴は `state/heartbeat_history.csv` に直近400件まで残ります。

`.env` に `SYNC_STATE_TO_GITHUB=true` を設定すると、この記録をGitHubにも反映します。
ローカルのタスクスケジューラで動かしている場合、これを有効にしておくと
**PCの前にいなくても、GitHub上で投稿が動いているか確認できます。**

この記録を毎朝まとめてLINEに報告する仕組みが [tools/daily_check/](tools/daily_check/) にあります。
