...
【1行目(フック) — リーチを決める最重要部分】
1行目の役割は「まだこのアカウントを知らない人のスクロールを止めること」。
板の静かな描写から入ると伸びない(実測: 描写型はリーチ30〜260)。
実際に伸びた投稿(リーチ1,000〜3,334)は次の型だった。**毎回どれか1つを選び、使い回さない**:
 (a) 呼びかけ・名指し —「新潟の薪ストーブユーザーさま、薪足りてますか？」
     「変木好き集まれー！」のように、**誰に向けた話か**を最初に言う
 (b) 困りごと・本音 —「木材の置き場所なくて困ってます」のような、
     作り手側の素の事情。かっこつけない
 (c) 用途から入る —「木工するならまずは木材が必要だよねぇ」のように、
     読み手がやりたいこと側から入る
 (d) 出来事の報告 —「〇〇まで納品してきました」「早速のご購入ありがとうございます」
 (e) 季節・地域 — 今の時期ならではの話、新潟・下越などの地名
- 1行目に樹種名を入れられるとなお良い(買い手は樹種名で探しているため)
- 「〜が入荷しました」だけの単調な報告や、盛り上げるだけの空虚な煽りにはしない

【文章のトーン(2行目以降)】
- 2行目以降で、この板の**具体的な一点**を言い切る。写真から実際に見えるものを選ぶ
...
【構成】
- 1行目: 上記【1行目(フック)】の型から選んだ一文(単独で改行し目立たせる)
...
# 3本/日は131と5.9倍の差があった。自分の投稿同士でフォロワーのフィードを
# 食い合い、1本あたりの初速エンゲージメントが分散するため。
SCHEDULED_WINDOWS_JST = [(20, 5)]
